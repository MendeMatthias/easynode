...
impl Daemon {
    pub fn new(
        ...
        info!("{:?}", network_info);
        // BTX (btxd, a Bitcoin Knots v29 fork) numbers its version differently from Bitcoin Core:
        // 0.33.1 reports version 3301 and subversion "/BTX:0.33.1/", well below Bitcoin's 16_00_00
        // scheme. It is a modern node with every RPC electrs needs, so accept any BTX subversion and
        // keep the 0.16+ numeric gate only for a (misconfigured) non-BTX daemon.
        let is_btx = network_info.subversion.contains("BTX");
        if !is_btx && network_info.version < 16_00_00 {
            bail!(
        ...
        let mut sync_waits: u32 = 0;
        loop {
            let info = daemon.getblockchaininfo()?;

            // `blocks == headers` is deliberately NOT required.
            //
            // Since the MatMul v4.7 fork at height 185,000 the node tracks thousands of
            // competing unattested header branches (11k+ headers-only tips measured
            // 2026-08-14), so `headers` permanently exceeds `blocks` -- 188,961 vs 187,802
            // -- and that equality can never hold again. It made electrs UNRESTARTABLE:
            // any restart parked here forever without ever binding the REST port, which
            // turned a stale-but-serving explorer into a hard 502 outage.
            //
            // electrs indexes the ACTIVE chain; headers on branches it will never index
            // are not a readiness signal. `initialblockdownload` remains the real one.
            if !info.initialblockdownload.unwrap_or(false) {
                break;
            }

            // Bounded, not conditional. While the network is partitioned above the fork this
            // node can sit in IBD indefinitely (blocks 185,141 vs headers 189,068), because
            // every block past 185,000 needs a signed attestation and the archives supply
            // them at roughly one per minute. Blocking forever means the explorer is DOWN;
            // serving a known-behind tip means it is up and the freshness guard can label it.
            // A stale answer callers can detect beats a 502 they cannot.
            if sync_waits >= 12 {
                warn!(
                    "bitcoind still reports IBD after 60s ({}/{} blocks); starting anyway and \
                     serving the active chain as far as it is indexed",

#[cfg(not(feature = "liquid"))] // BTX consensus data structures (rust-btx), unifying on bitcoin 0.32.4
pub use rust_btx::{
    address, deserialize, script, Address, Block, BlockHash, BlockHeader, OutPoint,
    ScriptBuf as Script, Sequence, Transaction, TxIn, TxMerkleNode, TxOut, Txid,
};
...
impl Network {
    /// Map the electrs network enum onto the BTX consensus network (`rust_btx::Network`).
    /// BTX only defines mainnet/testnet3/regtest, so testnet4 and signet fold onto testnet.
    #[cfg(not(feature = "liquid"))]
    pub fn btx(self) -> rust_btx::Network {
        match self {
            Network::Bitcoin => rust_btx::Network::Bitcoin,
            Network::Testnet | Network::Testnet4 | Network::Signet => rust_btx::Network::Testnet,
            Network::Regtest => rust_btx::Network::Regtest,
        }
    }
...
pub fn genesis_hash(network: Network) -> BlockHash {
    #[cfg(not(feature = "liquid"))]
    return btx_genesis_hash(network);
    #[cfg(feature = "liquid")]
    return liquid_genesis_hash(network);
}

#[cfg(not(feature = "liquid"))]
pub fn btx_genesis_hash(network: Network) -> BlockHash {
    // Mainnet/testnet3 have fixed BTX genesis hashes; regtest's is runtime-dependent, so fall
    // back to bitcoin's regtest genesis (only used for Electrum discovery, inactive on regtest).
    network
        .btx()
        .genesis_hash()
        .unwrap_or_else(|| bitcoin_genesis_hash(network.into()))
}
...
pub fn bitcoin_genesis_hash(network: BNetwork) -> bitcoin::BlockHash {
    ...
    match network {
        ...
        // `bitcoin::Network` is `#[non_exhaustive]`; a wildcard is required by rustc.
        _ => unreachable!("unsupported bitcoin network variant"),
    }
}

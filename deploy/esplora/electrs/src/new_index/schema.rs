...
impl ChainQuery {
    ...
    pub fn get_merkleblock_proof(&self, txid: &Txid) -> Option<MerkleBlock> {
        ...
        // `bitcoin::MerkleBlock` embeds a classic 80-byte `bitcoin::block::Header`, so project
        // the BTX 182-byte header onto its six classic fields. The partial merkle proof is
        // validated against `merkle_root`, which is carried faithfully; the txid branch/flags
        // are correct. NOTE: the embedded header's `block_hash()` is the classic-form hash, not
        // BTX's 182-byte hash (see integration review notes).
        let h = headerentry.header();
        let classic_header = bitcoin::block::Header {
            version: h.version,

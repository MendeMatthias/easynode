...
impl ScriptToAddr for bitcoin::Script {
    fn to_address_str(&self, network: Network) -> Option<String> {
        // Witness programs (incl. BTX witness-v2 P2MR → `btx1z…`) render with the BTX HRP.
        rust_btx::address::render_from_script(self, network.btx()).or_else(|| {
            // Legacy base58check scripts (P2PKH/P2SH) fall back to the shared renderer.

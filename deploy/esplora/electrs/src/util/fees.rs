...
pub struct TxFeeInfo {
    ...
    pub vsize: u64,         // in virtual bytes (BTX: WSF=1, so vsize == weight)
...
impl TxFeeInfo {
    pub fn new(tx: &Transaction, prevouts: &HashMap<u32, &TxOut>, network: Network) -> Self {
        ...
        // BTX has WITNESS_SCALE_FACTOR == 1: no witness discount, so
        // vsize == weight == total serialized size. rust_btx::Transaction::weight()
        // already implements WSF=1, so we must NOT divide by 4 here (that would make
        // /mempool fee_histogram and per-tx fee-rates 4x wrong). Liquid keeps WSF=4.
        #[cfg(not(feature = "liquid"))]
        let vsize_float = weight as f64; // WSF=1 identity (vsize == weight)
        #[cfg(feature = "liquid")]

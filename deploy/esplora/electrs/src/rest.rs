...
// BTX PQ txs are large (ML-DSA sigs ~2420B; standard tx weight up to 1_200_000
// under WITNESS_SCALE_FACTOR=1) and POST /tx bodies are hex-encoded (2x). Raised
// from 1_000_000 to ~25MB so legitimate large BTX txs can be broadcast.
const MAX_BODY_SIZE: usize = 25_000_000;
...
fn address_to_scripthash(addr: &str, network: Network) -> Result<FullHash, HttpError> {
    #[cfg(not(feature = "liquid"))]
    {
        // BTX bech32/bech32m (`btx1…`, incl. witness-v2 P2MR): the HRP is checked against the
        // network inside `parse`, so a wrong-network address fails here as before.
        if let Ok(script_pubkey) = rust_btx::address::parse(addr, network.btx()) {
            return Ok(compute_script_hash(&script_pubkey));
        }
        // Legacy base58check addresses keep the shared machinery + explicit network check.
